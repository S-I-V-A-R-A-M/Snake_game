...
const char* ssid ="SRK";
const char* password="SIVA@1112004";
const char* serverName="http:// 192.168.230.80/PHP/score.php";
...
void loop()
{
  ...
  while(k==2)
  {
        ...
        if(Row[1]>8 || Col[1]>8 || Row[1]<0 || Col[1]<0)
        {
         ...
         if(WiFi.status()==WL_CONNECTED && !requestSent/* && nice*/)
         {
    WiFiClient client;
    HTTPClient http;
    ...
    String httpRequestData = "score="+String(score)/*+"&"+"player_name="+ player_name*/;
    int httpResponseCode=http.POST(httpRequestData);
    Serial.println("Sending HTTP POST request with data: " + httpRequestData);
    ...
    Serial.println("WiFi Disconnected or Request Already Sent");
  }
